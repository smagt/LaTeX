...
Second, the above rules may be rules, but English is not that strict.  Being consistent throughout a piece of text is certainly advisable, since it reduces reader confusion.  But take the above dashes-piece.  Yes, it may be correct.  But if there is no confusion, as in ``two dimensional manifold,'' you need not be pedantic about putting the dash there.  You can, but leaving it out is not necessarily wrong.
...
$\KLf{p(x)}{q(x)}$.  There is ugly space after the KL; manual spacing around the $\|$, and  only limited control over the size of the parentheses.

The package \verb+mathtools+ gives an elegant solution, which we can exploit as follows:
\begin{verbatim}
