...
So, write ``the two-dimensional manifold'' since ``the two dimensional manifolds'' would mean something else (though I would not know what a ``dimensional manifold'' is).  Write the ``light-weight robot'' (although the whole name is clearly given by individuals who have no understanding of the English language: a weight is not light, but it can be low.  I don't want to get into that discussion, however, since ``light-weight'' is an agreed-upon anomaly which non-natives should not contemplate on.)\footnote{Was I right in putting this final full-stop before the parenthesis?}
